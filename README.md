...
# 📊 QuickBite: On-Demand Food Delivery Fleet Dispatch & Kitchen SLA Analytics

**On-Demand Food Delivery Logistics Data Warehouse: Courier dispatch ETA accuracy, dark kitchen preparation bottlenecks, batching density, and surge pricing elasticity.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/QuickBite-Food-Delivery-Fleet-Dispatch-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/QuickBite-Food-Delivery-Fleet-Dispatch-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/QuickBite-Food-Delivery-Fleet-Dispatch-Analytics)
...
**QuickBite-Food-Delivery-Fleet-Dispatch-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **Hyperlocal Delivery & Marketplace Intelligence**.
...
### 1️⃣ Executive KPI Command Center (Sunset Dark Ember (Dark Slate))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_riders       │──►│     fact_delivery_orders │◄──┤     dim_merchants    │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_geozones     │   │     dim_cuisine_types   │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM quickbite_dw.fact_delivery_orders f
JOIN quickbite_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/QuickBite-Food-Delivery-Fleet-Dispatch-Analytics.git
cd QuickBite-Food-Delivery-Fleet-Dispatch-Analytics
